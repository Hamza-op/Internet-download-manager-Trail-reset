...
pub fn fix_popup() {
    ...
    // 2. Neutralize IDMGrHlp.exe
    neutralize_idmgrhlp();
}

pub fn neutralize_idmgrhlp() {
    let hkcu = RegKey::predef(HKEY_CURRENT_USER);
    ...
    let integrator_path = idm_path.join("IDMGrHlp.exe");
    if integrator_path.exists() {
        // Check size
        if let Ok(metadata) = std::fs::metadata(&integrator_path) {
            if metadata.len() == 0 {
                debug_print("  [✓] IDMGrHlp.exe is already neutralized.");
                return;
            }
        }
        ...
        match std::fs::rename(&integrator_path, &backup_path) {
            Ok(_) => debug_print("  [✓] IDMGrHlp.exe renamed to .bak"),
            Err(e) => {
        ...
        match std::fs::write(&integrator_path, "") {
            Ok(_) => {
                let _ = crate::hidden_command("icacls")
                    ...
                    .output();
                debug_print("  [✓] IDMGrHlp.exe neutralized/replaced.");
            }
...
fn kill_idm() {
    let processes = ["idman.exe", "IDMGrHlp.exe"];
